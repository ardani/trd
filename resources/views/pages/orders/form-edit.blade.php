...
<fieldset class="form-group col-md-2">
    ...
    <input type="text" class="form-control" name="invoice_no" value="{{ $model->invoice_no }}">
</fieldset>
<fieldset class="form-group col-md-2">
...
<fieldset class="form-group col-md-3">
    ...
    <select name="supplier_id" class="form-control select-supplier" data-live-search="true">
        <option value="{{ $model->supplier_id }}">{{ $model->supplier->name }}</option>
    </select>
</fieldset>
<fieldset class="form-group col-md-2">
    ...
    <div class="input-group">
        <div class="input-group-addon">
            <input type="checkbox" {{$model->payment_method_id == 2 ? 'checked' : ''}}  name="payment_method_id"
                   value="2" id="check-1">
        </div>
        <input {{$model->payment_method_id == 2 ? '' : 'disabled'}}  type="text" id="paid-until-at" name="paid_until_at"
               value="{{$model->paid_until_at ? $model->paid_until_at->format('d/m/Y') : date('d/m/Y') }}"
               class="form-control daterange" placeholder="credit date">
    </div>
</fieldset>
<fieldset class="form-group col-md-2">
...
<fieldset class="form-group col-md-2">
    ...
    <div class="input-group">
        ...
        <div class="input-group-btn">
            <button type="button" id="save-btn" data-url="{{url('orders/actions/add')}}" class="btn btn-info"><span
        ...
        @foreach($model->transactions as $transaction)
            <tr>
                ...
                <td class="text-right subtotal"
                    data-content="subtotal">{{number_format($transaction->qty * $transaction->purchase_price * $transaction->attribute)}}</td>
                <td>
